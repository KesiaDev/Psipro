    ...
    private fun loadAllAppointments() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            appointmentViewModel.getAppointmentsByDateRange(today, endOfToday).collectLatest { appointments ->
                // Verificar novamente se o fragment ainda está ativo
                ...
                val confirmadas = todayAppointments.count { it.status == AppointmentStatus.CONFIRMADO }
                val faltas = todayAppointments.count { it.status == AppointmentStatus.FALTOU }

                binding.appointmentsCountText.text = totalHoje.toString()
                binding.tvAtendidas.text = confirmadas.toString()
