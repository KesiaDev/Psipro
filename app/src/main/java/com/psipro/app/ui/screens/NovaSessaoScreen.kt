...
fun NovaSessaoScreen(
    ...
    onSave: () -> Unit,
    onCancel: () -> Unit,
    onAgendarReconsulta: (Long, String) -> Unit = { _, _ -> },
    ...
    val cobrancaPendente = remember(anotacaoId, viewModel.anotacoes.value) {
        if (anotacaoId > 0) {
            viewModel.anotacoes.value.find { it.id == anotacaoId }?.let { anotacao ->
                runBlocking {
                    viewModel.getStatusCobrancaPorAnotacao(anotacao.id) == com.psipro.app.data.entities.StatusPagamento.A_RECEBER
                }
            }
        } else false
    }
    ...
    Box(Modifier.fillMaxSize()) {
        Card(
            ...
            Column(
                ...
                // Botão para agendar reconsulta
                Button(
                    onClick = { onAgendarReconsulta(patientId, "Reconsulta") },
                    shape = botaoShape,
                    modifier = Modifier.fillMaxWidth(),
