package com.psipro.app.reports
...
class AppointmentReportGenerator @Inject constructor() {
    ...
    fun generatePatientReport(
        ...
        val completedAppointments = appointments.count { it.status == AppointmentStatus.REALIZADO }
        val cancelledAppointments = appointments.count { it.status == AppointmentStatus.CANCELOU }
        val upcomingAppointments = appointments.count { 
            it.status == AppointmentStatus.CONFIRMADO && it.date.after(Date())
        }
