"use client";

import { useEffect, useMemo, useState, useCallback } from "react";
import { useClinic } from "../contexts/ClinicContext";
import { useOnboarding } from "../contexts/OnboardingContext";
import { usePathname } from "next/navigation";
import { isFirstAccess } from "../utils/onboarding";
import { generateInsights } from "../insights/InsightEngine";
import type { InsightInput } from "../insights/types";
import MetricCard from "../components/dashboard/MetricCard";
import SectionCard from "../components/dashboard/SectionCard";
import ActionCard from "../components/dashboard/ActionCard";
import RoleBadge from "../components/RoleBadge";
import OnboardingModal from "../components/onboarding/OnboardingModal";
import { dashboardService } from "../services/dashboardService";
import Skeleton from "../components/Skeleton";

import { DASHBOARD_REFRESH_EVENT } from "../constants/events";

export default function DashboardPage() {
  const { currentClinic, isIndependent } = useClinic();
  const { openOnboarding } = useOnboarding();
  const pathname = usePathname();

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<{
    patientsCount: number;
    appointmentsToday: { count: number; items: Array<{ id: string; patientId: string; scheduledAt: string; patient?: { name: string } }> };
    sessionsStats: { sessionsThisMonth: number; sessionsThisWeek: number };
    financial: { receitaHoje: number; receitaMes: number; totalAReceber: number; ticketMedio: number };
  } | null>(null);

  const loadDashboardData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const [patientsCount, appointmentsToday, sessionsStats, financial] =
        await Promise.all([
          dashboardService.getPatientsCount(),
          dashboardService.getAppointmentsToday(),
          dashboardService.getSessionsStats(),
          dashboardService.getFinancialSummary(),
        ]);

      setData({
        patientsCount,
        appointmentsToday,
        sessionsStats,
        financial,
      });
    } catch (e) {
      const msg =
        e && typeof e === "object" && "message" in e
          ? String((e as { message: string }).message)
          : "Erro ao carregar dados do dashboard";
      setError(msg);
      setData(null);
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    loadDashboardData();
  }, [currentClinic?.id, loadDashboardData]);

  useEffect(() => {
    const handler = () => loadDashboardData();
    window.addEventListener(DASHBOARD_REFRESH_EVENT, handler);
    return () => window.removeEventListener(DASHBOARD_REFRESH_EVENT, handler);
  }, [loadDashboardData]);

  useEffect(() => {
    if (pathname === "/dashboard") {
      loadDashboardData();
    }
  }, [pathname, loadDashboardData]);

  useEffect(() => {
    if (isFirstAccess()) {
      const timer = setTimeout(() => openOnboarding(), 500);
      return () => clearTimeout(timer);
    }
  }, [openOnboarding]);

  const insights = useMemo(() => {
    if (!data) return [];
    const insightData: InsightInput = {
      sessionsThisMonth: data.sessionsStats.sessionsThisMonth,
      sessionsLastMonth: 0,
      sessionsThisWeek: data.sessionsStats.sessionsThisWeek,
      scheduledSessionsNextWeek: data.appointmentsToday.count,
      missedSessions: 0,
      cancelledSessions: 0,
      monthlyRevenue: data.financial.receitaMes,
      lastMonthRevenue: 0,
      averageRevenuePerSession: data.financial.ticketMedio,
      unpaidSessions: 0,
      totalPendingRevenue: data.financial.totalAReceber,
      activePatients: data.patientsCount,
      newPatientsThisMonth: 0,
      patientsWithoutSessions: 0,
      patientsWithManySessions: 0,
    };
    const generated = generateInsights(insightData);
    const priorityOrder: Record<string, number> = {
      warning: 4,
      tip: 3,
      success: 2,
      info: 1,
    };
    const prioritized = generated
      .sort((a, b) => priorityOrder[b.type] - priorityOrder[a.type])
      .slice(0, 3);
    if (prioritized.length === 0) {
      return [
        {
          id: "default-tip",
          type: "tip" as const,
          category: "geral" as const,
          title: "Começando a usar o PsiPro",
          description:
            "Conforme você usar o app e registrar sessões, insights personalizados aparecerão aqui automaticamente.",
          priority: 1,
        },
      ];
    }
    return prioritized;
  }, [data]);

  const formatCurrency = (value: number) =>
    new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(value);

  const formatTime = (iso: string) => {
    try {
      return new Date(iso).toLocaleTimeString("pt-BR", {
        hour: "2-digit",
        minute: "2-digit",
      });
    } catch {
      return iso;
    }
  };

  if (error) {
    return (
      <>
        <OnboardingModal />
        <div className="w-full min-w-0 py-6 sm:py-8">
          <div className="bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 rounded-lg p-6">
            <p className="font-medium mb-2">Erro ao carregar o dashboard</p>
            <p className="text-sm opacity-90">{error}</p>
            <button
              onClick={loadDashboardData}
              className="mt-4 px-4 py-2 text-sm font-medium bg-red-500/20 hover:bg-red-500/30 rounded-lg transition-colors"
            >
              Tentar novamente
            </button>
          </div>
        </div>
      </>
    );
  }

  return (
    <>
      <OnboardingModal />
      <div className="w-full min-w-0 py-6 sm:py-8">
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-4">
            <div className="min-w-0">
              <h1 className="text-2xl sm:text-3xl font-bold text-psipro-text mb-2 tracking-tight">
                Dashboard
              </h1>
              <p className="text-psipro-text-secondary text-base sm:text-lg">
                Visão geral da sua clínica
              </p>
            </div>
            {currentClinic && (
              <div className="text-left sm:text-right shrink-0">
                <p className="text-sm text-psipro-text-secondary mb-1 truncate">
                  {currentClinic.name}
                </p>
                <RoleBadge role={currentClinic.role || ""} />
              </div>
            )}
            {isIndependent && (
              <div className="text-left sm:text-right shrink-0">
                <p className="text-sm text-psipro-text-secondary">Modo Independente</p>
              </div>
            )}
          </div>
        </div>

        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5 mb-8">
            {[1, 2, 3, 4, 5].map((i) => (
              <Skeleton key={i} className="h-32" />
            ))}
          </div>
        ) : data ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5 mb-8">
              <MetricCard
                title="Pacientes ativos"
                value={data.patientsCount}
                icon="👥"
              />
              <MetricCard
                title="Sessões realizadas"
                value={data.sessionsStats.sessionsThisMonth}
                icon="📅"
                subtitle="Este mês"
              />
              <MetricCard
                title="Consultas hoje"
                value={data.appointmentsToday.count}
                icon="📋"
              />
              <MetricCard
                title="Receita do mês"
                value={formatCurrency(data.financial.receitaMes)}
                icon="💰"
              />
              <MetricCard
                title="Valores a receber"
                value={formatCurrency(data.financial.totalAReceber)}
                icon="⏳"
              />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              <SectionCard
                title="Agenda da clínica"
                isEmpty={data.appointmentsToday.count === 0}
                emptyState={{
                  icon: "📅",
                  title: "Nenhuma consulta agendada para hoje",
                  description:
                    "As consultas de hoje aparecem aqui assim que forem agendadas no app ou na agenda web.",
                  hint: "A criação e confirmação de sessões é feita no app PsiPro ou na agenda.",
                }}
              >
                <div className="space-y-4">
                  <div>
                    <p className="text-sm text-psipro-text-secondary mb-2">
                      Consultas de hoje
                    </p>
                    <div className="space-y-2">
                      {data.appointmentsToday.items.map((apt) => (
                        <div
                          key={apt.id}
                          className="flex items-center justify-between p-3 bg-psipro-surface rounded-lg"
                        >
                          <span className="text-sm text-psipro-text">
                            {apt.patient?.name || "Paciente"}
                          </span>
                          <span className="text-sm font-semibold text-psipro-text">
                            {formatTime(apt.scheduledAt)}
                          </span>
                        </div>
                      ))}
                    </div>
                  </div>
                </div>
              </SectionCard>

              <SectionCard
                title="Resumo financeiro"
                isEmpty={data.financial.receitaMes === 0 && data.financial.totalAReceber === 0}
                emptyState={{
                  icon: "💰",
                  title: "Aguardando dados financeiros",
                  description:
                    "Os dados financeiros são gerados a partir das sessões realizadas no app.",
                  hint: "A web é usada para acompanhar e analisar.",
                }}
              >
                <div className="space-y-4">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-4 bg-psipro-surface rounded-lg border border-psipro-border">
                      <div>
                        <p className="text-sm text-psipro-text-secondary mb-1">
                          Receita total do mês
                        </p>
                        <p className="text-2xl font-bold text-psipro-text">
                          {formatCurrency(data.financial.receitaMes)}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center justify-between p-4 bg-psipro-surface rounded-lg border border-psipro-border">
                      <div>
                        <p className="text-sm text-psipro-text-secondary mb-1">
                          Ticket médio
                        </p>
                        <p className="text-xl font-semibold text-psipro-text">
                          {formatCurrency(data.financial.ticketMedio)}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center justify-between p-4 bg-psipro-surface rounded-lg border border-psipro-border">
                      <div>
                        <p className="text-sm text-psipro-text-secondary mb-1">
                          Valores a receber
                        </p>
                        <p className="text-xl font-semibold text-psipro-text">
                          {formatCurrency(data.financial.totalAReceber)}
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </SectionCard>
            </div>

            <div className="mb-8">
              <div className="mb-4">
                <h2 className="text-xl font-semibold text-psipro-text mb-2">
                  Insights do PsiPro
                </h2>
                <p className="text-sm text-psipro-text-secondary">
                  Observações automáticas sobre sua prática clínica
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {insights.map((insight) => (
                  <div
                    key={insight.id}
                    className="bg-psipro-surface-elevated rounded-lg border border-psipro-border p-4"
                  >
                    <div className="flex items-start gap-3">
                      <span className="text-xl shrink-0">
                        {insight.type === "warning"
                          ? "⚠️"
                          : insight.type === "success"
                            ? "✅"
                            : insight.type === "tip"
                              ? "💡"
                              : "ℹ️"}
                      </span>
                      <div className="flex-1">
                        <h3 className="font-semibold text-psipro-text mb-1 text-sm">
                          {insight.title}
                        </h3>
                        <p className="text-sm text-psipro-text-secondary leading-relaxed">
                          {insight.description}
                        </p>
                      </div>
                    </div>
                  </div>
                ))}
              </div>
            </div>

            <ActionCard
              title="O que fazer agora"
              description="Ações sugeridas para começar a usar o PsiPro"
              actions={[
                { label: "Importar pacientes", href: "/pacientes", variant: "primary" },
                { label: "Gerenciar clínica", href: "/clinica", variant: "secondary" },
              ]}
            />
          </>
        ) : null}
      </div>
    </>
  );
}
